...
# FILE: run-local-icodex.sh
# Purpose: Starts a local relay plus the local-first iCodex bridge for source installs and LAN testing.
# Layer: developer utility
# Exports: none
# Depends on: node, npm, curl, relay/server.js, phodex-bridge/bin/icodex.js
...
log() {
  echo "[run-local-icodex] $*"
}

die() {
  echo "[run-local-icodex] $*" >&2
  exit 1
}

usage() {
  cat <<'EOF'
Usage: ./run-local-icodex.sh [options]
...
cleanup() {
  if [[ "${BRIDGE_SERVICE_STARTED}" == "true" ]]; then
    (
      cd "${BRIDGE_DIR}"
      node ./bin/icodex.js stop >/dev/null 2>&1 || true
    )
  fi
...
print_summary() {
  cat <<EOF
[run-local-icodex] Configuration
...
start_bridge() {
  log "Starting bridge"
  cd "${BRIDGE_DIR}"
  ICODEX_RELAY="ws://${RELAY_HOSTNAME}:${RELAY_PORT}/relay" node ./bin/icodex.js up
  BRIDGE_SERVICE_STARTED="true"
}

hold_open() {
  log "Local relay is ready. Keep this terminal open while testing."
  log "Press Ctrl+C to stop both the local relay and the iCodex bridge service."
  wait "${RELAY_PID}"
}
