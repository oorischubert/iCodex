...
function watchThreadRollout(threadId = "") {
  ...
  console.log(`[icodex] Watching thread ${resolvedThreadId}`);
  console.log(`[icodex] Rollout file: ${rolloutPath}`);
  console.log("[icodex] Waiting for new persisted events... (Ctrl+C to stop)");
...
function resolveThreadId(threadId) {
  ...
  throw new Error("No thread id provided and no remembered iCodex thread found.");
}
